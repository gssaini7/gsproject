 [Authorize]
    ...
    {
        iLayoutService _mainobj;
      
        public LayoutController(iLayoutService imainobj)
        {
        ...
        [Route("Create")]
        [HttpPost]
        public async Task<IHttpActionResult> SaveDetail(LayoutModel model)
        {
            try
            {
                if (model == null)
                    return null;
                ModelState.Remove("model.LayoutModelid");
                if (!ModelState.IsValid)
                {
                    return BadRequest(ModelState);
                }

                model.LayoutModelid =Guid.NewGuid();
        ...
        public HttpResponseMessage GetByID(string id)
        {
            ...
            if (webmanager!=null)
            {
                var deserializedProduct = JSONGS<LayoutModel>(webmanager);
        ...
        public HttpResponseMessage GetDetail()
        {
            ...
            if (webmanager != null)
            {
                var deserializedProduct = JSONGS<IEnumerable<LayoutModel>>(webmanager);
                return Request.CreateResponse(HttpStatusCode.OK, deserializedProduct);
            }
        ...
        [Route("Edit")]
        [HttpPost]
        public async Task<IHttpActionResult> EditDetail(LayoutModel model)
        {
            var gid = model.LayoutModelid;
            var dbmanager = _mainobj.GetById(gid);
            if (dbmanager != null)
            {
			 	 dbmanager.LayoutModelid = model.LayoutModelid;
	 dbmanager.LayoutName = model.LayoutName;
