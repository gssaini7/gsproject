...
export class MainDatabasesComponent implements OnInit {
    ...
    InitilizeFormItems() {
        this.mainFrm = this.fb.group({
            ...
            IDataSource: ['', Validators.required],
            IInitialCatalog: ['', Validators.required],
            IUsername: [''],
            IPassword: [''],
            IName: ['', Validators.required],
            IUIDCode: ['', Validators.required],
            isPublished: [''],
            remarks: [''],
        });
    }
    ...
    LoadData(): void {
        ...
        this._crudService.get(Global.BASE_USER_ENDPOINT + "institutes/GetAll")
            .subscribe(records => {
    ...
    onSubmit(formData: any) {
        ...
        switch (this.dbops) {
            case DBOperation.create:
                ...
                var curl = Global.BASE_USER_ENDPOINT + "institutes/Create";
            ...
            case DBOperation.update:
                var curl = Global.BASE_USER_ENDPOINT + "institutes/Edit";
                this._crudService.post(curl, formData._value).subscribe(
                    data => {
                        if (data == 1) //Success
                        {
            ...
            case DBOperation.delete:
                var curl = Global.BASE_USER_ENDPOINT + "institutes/Delete";
